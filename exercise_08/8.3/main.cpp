...
{
  return 1. * pow(val,4) + 2.5 * pow(val,2);
}
...
double variationalWaveFunction(double x)
{
  ...
  */
	return 1.0; // uncomment this for constant wavefunction
}
...
double variationalWaveFunction_second(double x)
{
  ...
  return exp(-0.5 *pow((x-mu)/sigma,2)) * (mu*mu - sigma*sigma + x*x - 2.*mu*x)/pow(sigma,4) +
         exp(-0.5 *pow((x+mu)/sigma,2)) * (mu*mu - sigma*sigma + x*x + 2.*mu*x)/pow(sigma,4);
  */
  return 0; // uncomment this for constant wavefunction
}
