...
void Input(string input_file){ //Prepare all stuff for the simulation
  ...
  ReadInput >> temp;
  cout << "System temperature = " << temp << endl;
  ReadInput >> npart;
  ...
  cout << "The system configuration files are read and saved in the \'config\' folder"<<endl;
  ...
  cout << "Rescaling velocity to match temperature"<<endl;
...
void Measure(){ //Properties measurement
  ...
  stima_pres = (rho * stima_temp + p / (3. * vol));  //Total pressure
...
void ConfFinal(void){ //Write final configuration
  ...
  //if(old == 1)
  {
    ...
    WriteOldConf.open("config/old_config.final");
...
void Averages(int iblk) //Print results for current block
{
    ...
    Gofr.open("data/g.out",ios::app);
    Gave.open("data/ave_g.out");
